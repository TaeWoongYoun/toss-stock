...
| 📊 **시세** | 현재가(등락률 포함) · 호가 · 체결내역 · 캔들 · 상/하한가 · 종목정보 · 매수유의 |
| 🏆 **순위** | 거래대금 · 거래량 · 시가총액 · 상승률 · 하락률 · 인기검색 (코스피/코스닥/통합) |
...
- **국장 ETF · 순위**: 네이버 금융 (ETF 목록 / 종목 순위)
- **미장 종목**: [NASDAQ Trader Symbol Directory](https://www.nasdaqtrader.com/Trader.aspx?id=symboldirdefs)

> ℹ️ 시세·주문·계좌는 **토스증권 공식 API**를 사용합니다. 순위·종목목록 등 토스 API 가 제공하지 않는
> 보조 데이터만 네이버/거래소 공개 데이터로 보완합니다. (조회 전용, 주문과 무관)
